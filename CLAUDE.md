...
| `EurekaUsage` | Nine incremental+dedup usage scanners — file-based ones tail transcripts/rollouts; `Opencode`/`Antigravity`/`Hermes` read SQLite instead (Hermes diffs `session_model_usage` against a per-session snapshot). CodeBuddy's usage rides on its `function_call` transcript lines (`providerData.usage`); Qoder is excluded (CN backend reports zero tokens). Plus `PricingTable`, `RateLimitProvider` protocol + Codex/Claude impls. |
