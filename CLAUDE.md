...
| `EurekaUsage` | Eleven incremental+dedup usage scanners — file-based ones tail transcripts/rollouts; `Opencode`/`Hermes`/`Cursor` read SQLite instead (Hermes diffs `session_model_usage` against a per-session snapshot; Cursor differences adjacent `tokenCount.inputTokens` because it re-sends the whole context each turn and reports no cache split; all `cursor/*` models are priced `unknown` → tokens counted, cost $0). Grok's rides on `updates.jsonl` `turn_completed` lines (per-turn `modelUsage`; `inputTokens` includes cache reads) and stores Grok's own `costUsdTicks / 1e10` in `reported_cost_usd`, which `PricingTable.cost(of:)` prefers over any catalog price; a rewritten `updates.jsonl` (new inode) is rescanned from zero after deleting that session's grok rows — never rely on dedup keys, they're pruned after 8 days. Antigravity has no usage scanner (conversations are protobuf, no local token accounting). CodeBuddy's usage rides on its `function_call` transcript lines (`providerData.usage`); Qoder is excluded (CN backend reports zero tokens), and so is Trae (session DB is SQLCipher-encrypted → no local token accounting at all). Plus the pricing layer under `Pricing/` (`PriceResolver` over the LiteLLM + models.dev catalogs, wrapped by `PricingTable` / `PricingCatalogStore`), `RateLimitProvider` protocol + Codex/Claude/Grok impls. |
...
- **Stale-event suppression:** events older than 5 minutes only enter history/usage; they must NOT trigger island animations (`AppDelegate.handle` drops stale heartbeat/waiting/session-start events entirely). Every live channel must compute `isStale` from the event's **own** timestamp — the opencode tailer hardcoded `false` until 0.31.0. Watermark tailers (opencode `event.rowid`) must treat a failed watermark query as "skip this tick", never as a regression, and re-baseline without replay when the table really shrinks: one failed `MAX(rowid)` read replayed 25k historical events as live and queued hundreds of island cards. The card queue additionally caps pending finished/notice cards (`IslandCardQueue.maxPendingPassive`) so no burst can pin the island open.
...
- **Prices are resolved at read time, never stored.** `usage_records` holds tokens (+ an optional `provider`); `PricingTable.cost(of:)` prices each `(model, provider)` aggregate through `PriceResolver`: user `pricing.json` → hard-unknown (`cursor/`, `authoritative: true`) → provider hint (only third-party hosts; subscription plans' $0 is never trusted) → LiteLLM exact key → vendor family on models.dev / LiteLLM `<vendor>/` keys → bundled exact → bundled prefix (flagged *estimated*). Subscription plans are priced at the vendor's pay-as-you-go rate (API-equivalent, same as Claude Max via ccusage). **Reseller prices (openrouter, qiniu-ai, poe…) are never used unless the user's provider *is* that reseller** — one `glm-5.2` ranges from $0.3 to $1.8 across them. A new catalog arriving hot-swaps `PricingCatalogStore.shared`, so historic costs re-price automatically. Fetch failures must keep the previous catalog (cache → bundled snapshot → handwritten table), never blank prices; `make pricing-catalog` refreshes the snapshot before each release.
...
App data lives in `~/Library/Application Support/Eureka/`: `eureka.sqlite` (history/usage/sessions), `events/` (spool), `bin/eureka-relay` (stable path), optional `pricing.json` (top-priority price overrides + provider aliases), `pricing-catalog-cache.json` (last fetched LiteLLM + models.dev catalog) and `context-windows.json` (override per-model context-window size, the denominator for ctx%). Config backups are written as `*.bak.eureka.*` before any hooks/notify edit.
