    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Theme.spacing.module) {
                ...
                Text("Codex/Grok 限额来自本地日志快照（零网络请求）；Claude 限额走非官方接口，失效时自动隐藏。OpenCode / Antigravity / Kimi / Gemini / Qwen / Hermes 无本地限额数据源（Kimi 配额仅官网会员页可见；Hermes 经各 provider 订阅计费，本地不落配额快照），故不显示。")
                    .font(.system(size: 10))
